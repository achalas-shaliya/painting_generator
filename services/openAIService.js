...
const { emitStatus } = require("./socketService");
...
async function generateImage(ideaId, prompt, references = []) {
  ...
  try {
    ...
    await pool.execute(
      "UPDATE paintings SET status = ?, image_url=? WHERE idea_id = ?",
      statusUpdateParams
    );
    console.log(`Updated status to processing for idea ${ideaId}`);

    // Emit status to socket
    emitStatus("thumbnailStatus", {
      titleId: ideaId,
      status: "processing",
    });
    ...
    try {
      ...
      const failureUpdateParams = [
        ...
        "https://placehold.co/600x400/7f8c8d/ffffff?text=Failed",
        ideaId,
      ];
      // Validate parameters
      if (failureUpdateParams.some((p) => p === undefined)) {
        ...
        await pool.execute(
          "UPDATE paintings SET status = ?, error_message = ?, image_url = ? WHERE idea_id = ?",
          failureUpdateParams
        );
